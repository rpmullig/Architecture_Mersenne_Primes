...
// This is used frequently after a mathematical operation is performed.
...
Bigint sub_big(Bigint a, Bigint b)
{
	Bigint c;
	/* my code -- assume a > b */
    // initialize digits of c, our return value, as largest number
	// fill in zeros for our return value
    c.n = a.n;
	for(int i = 0; i < c.n; i++){
        c.digits[i] = a.digits[i]; // overwrite with digits from a
        if(i >= b.n){
            b.digits[i] = 0;
        }
    }
	...
	int i = 0;
    while(i < a.n){
        k = c.digits[i] - b.digits[i];
        if(k >= 0){
            c.digits[i] = k;
        } else{
            c.digits[i+1] = c.digits[i+1] - 1; // take the carry
            c.digits[i] = 10 + c.digits[i] - b.digits[i];
        }
        i += 1;
    }
    compress(&c); // Trim any leading zeros
